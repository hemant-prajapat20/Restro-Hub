...
function fixPhoneInputRobust() {
  ...
  for (const file of files) {
    ...
    const regex = /<input\s+type="text"\s+placeholder="Mobile Number"\s+value=\{customerPhone\}\s+onChange=\{\(e\) => setCustomerPhone\(e\.target\.value\)\}\s+className="[^"]+"\s*\/>/;
    ...
    if (regex.test(content)) {
        ...
        console.log('Fixed phone input robustly in', file);
    }
  }
}
