    ...
    <div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-brand-accent/30 mb-8">
          ...
          <form onSubmit={handleSaveAddress} className="space-y-4">
            ...
            <div className="flex gap-4 mb-2">
                {['Home', 'Work', 'Other'].map(lbl => (
                    <button 
                        ...
                        className={`px-4 py-2 rounded-xl font-bold text-sm flex items-center gap-2 transition-all ${newAddress.label === lbl ? 'bg-brand-primary text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                    >
            ...
            <div>
                ...
                <input required type="text" value={newAddress.street} onChange={e => setNewAddress({...newAddress, street: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-medium text-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-accent/50" placeholder="e.g. 123 Main St, Apt 4B" />
            </div>
            
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">City</label>
                    <input required type="text" value={newAddress.city} onChange={e => setNewAddress({...newAddress, city: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-medium text-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-accent/50" placeholder="e.g. Jaipur" />
                </div>
                <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">State</label>
                    <input required type="text" value={newAddress.state} onChange={e => setNewAddress({...newAddress, state: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-medium text-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-accent/50" placeholder="e.g. Rajasthan" />
                </div>
            </div>
            
            <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">Zip / Postal Code</label>
                <input required type="text" value={newAddress.zipCode} onChange={e => setNewAddress({...newAddress, zipCode: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-medium text-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-accent/50" placeholder="e.g. 302001" />
            </div>

            <div className="flex items-center gap-2 mt-4">
               <input type="checkbox" id="isDefault" checked={newAddress.isDefault} onChange={e => setNewAddress({...newAddress, isDefault: e.target.checked})} className="w-5 h-5 rounded text-brand-accent focus:ring-brand-accent" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {addresses.map((address) => (
          <div key={address._id} className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 hover:border-brand-accent/30 transition-colors relative group">
             {address.isDefault && (
             ...
             <div className="flex items-start gap-4">
                 <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 ${address.label.toLowerCase() === 'home' ? 'bg-blue-50 text-blue-500' : address.label.toLowerCase() === 'work' ? 'bg-purple-50 text-purple-500' : 'bg-brand-accent/10 text-brand-accent'}`}>
        ...
        {addresses.length === 0 && !showAddForm && (
            <div className="col-span-full py-12 text-center bg-white rounded-3xl border border-slate-100 shadow-sm border-dashed">
                ...
                <h3 className="text-lg font-bold text-brand-primary mb-1">No addresses saved</h3>
                <p className="text-slate-500 font-medium mb-4">Add your delivery addresses for a faster checkout.</p>
                <button onClick={() => setShowAddForm(true)} className="text-brand-accent font-bold hover:underline">Add New Address</button>
            </div>
        )}
      </div>
    </div>
